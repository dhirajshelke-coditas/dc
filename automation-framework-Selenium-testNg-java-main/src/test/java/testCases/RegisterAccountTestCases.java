...
public class RegisterAccountTestCases extends SuiteSetup {
	...
	@Test(priority = 3, description = "Verify registration fails with password mismatch", groups = { "register",
			"field-validation" })
	public void verifyRegistrationWithPasswordMismatch() {
		log.info("TC-R03: Verify registration with mismatched passwords");

		homePage.clickMyAccounts();
		homePage.clickRegister();

		String email = "mismatch" + System.currentTimeMillis() + "@mail.com";
		registerPage.registerWithPasswordMismatch("Test", "User", email, "9876543210", "Test@1234", "Different@9999");

		boolean errorShown = registerPage.isPasswordMismatchErrorDisplayed();
		Assert.assertTrue(errorShown, "TC-R03 FAILED: Password mismatch error not displayed");
		log.info("TC-R03 PASSED");
	}

	@Test(priority = 4, description = "Verify registration fails when all fields are empty", groups = { "register",
			"field-validation" })
	public void verifyRegistrationWithAllFieldsEmpty() {
		log.info("TC-R04: Verify registration with all fields empty");
		...
		Assert.assertTrue(errorsShown, "TC-R04 FAILED: Validation errors not shown for empty fields");
		log.info("TC-R04 PASSED");
	}

	@Test(priority = 5, description = "Verify registration with invalid email format", groups = { "register",
			"field-validation" })
	public void verifyRegistrationWithInvalidEmailFormat() {
		log.info("TC-R05: Verify registration with invalid email format");
		...
		Assert.assertTrue(onRegisterPage, "TC-R05 FAILED: Should stay on register page for invalid email");
		log.info("TC-R05 PASSED");
	}

	@Test(priority = 6, description = "Verify registration without accepting privacy policy", groups = { "register",
			"field-validation" })
	public void verifyRegistrationWithoutPrivacyPolicy() {
		log.info("TC-R06: Verify registration without accepting privacy policy");
		...
		boolean onRegisterPage = registerPage.isOnRegisterPage();
		Assert.assertTrue(onRegisterPage, "TC-R06 FAILED: Should not register without policy agreement");
		log.info("TC-R06 PASSED");
	}

	@Test(priority = 7, description = "Verify registration with short (weak) password", groups = { "register",
			"field-validation" })
	public void verifyRegistrationWithWeakPassword() {
		log.info("TC-R07: Verify registration with weak/short password");
		...
		Assert.assertTrue(registerPage.isPasswordLengthErrorDisplayed(),
				"TC-R07 FAILED: 'Password must be between 4 and 20 characters' error not displayed");
		log.info("TC-R07 PASSED");
	}
}